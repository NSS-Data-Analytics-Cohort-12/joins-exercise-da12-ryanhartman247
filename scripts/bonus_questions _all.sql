...
SELECT 
	LEFT(CAST(release_year as VarChar), 3) || '0s' AS decade, -- changed year to string to use LEFT function. And || = concatenate. Also, would release_year/10*10 work here as well?? 
	SUM(worldwide_gross) AS total_ww_gross,
	ROUND(AVG(imdb_rating),2) AS avg_imdb
FROM specs s
	JOIN rating
	ON s.movie_id = rating.movie_id
	JOIN revenue
